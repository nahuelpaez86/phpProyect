...
<body class="pt-1">
  ...
  <div class="container-fluid px-3">
     <div class="container-fluid px-3">
            <a href="admin-users-add.php" class="btn btn-sm btn-primary">Agregar auto</a>
            <!-- … -->
        </div>
          ...
          <tbody>
             <?php if (!empty($cars)): ?>
                ...
                <?php if ($editingId == $car['id']): ?>
                    <form method="POST" action="services/carsServices.php?action=update&id=<?= $car['id'] ?>&path=admin-cars.php">
                        <input type="hidden" name="id" value="<?= $car['id'] ?>">
                        <tr>
                          <td><img height='100' width='150' src="<?= $car['image'] ?>"/></td>
                          <td><input type="text" name="title" class="form-control" value="<?= htmlspecialchars($car['title']) ?>" required ></td>
